...
def preprocess_input(text):
    ...
    corrections = [
        (r"\b(indian|india|indeedin|indie)\s*(inspiring|spring)\s*(infotech|info tech)?\b", "Indeed Inspiring Infotech"),
        (r"\b(give me|get me|i need)\s*(trees|tree|plants)\b", "Give Me Trees"),
        (r"\b(your org|your foundation|the foundation|this org)\b", "Give Me Trees Foundation")
    ]
    for pattern, replacement in corrections:
        text = re.sub(pattern, replacement, text, flags=re.IGNORECASE)
    return text
...
def get_gmtt_response(user_input):
    ...
    if any(phrase in processed_input for phrase in identity_phrases):
        kb_response = search_knowledge(processed_input, gmtt_kb)
        if kb_response:
            return kb_response
        if "name" in processed_input:
            return f"My name is {GMTT_NAME}."
        if "location" in processed_input or "address" in processed_input:
    ...
    # Fallback to Gemini
    return get_gemini_response(user_input, "Give Me Trees Foundation")
...
def get_gemini_response(query, context):
    try:
        ...
        prompt = f"""You are a helpful assistant for {context}. 
        Respond to this query in 1-2 sentences
        -Website: https://www.givemetrees.org
        - support email for website is info@givemetrees.org
        -If unsure, say:
        "I'll need to check on that. Please visit our website for more information."
...
# -------------------- Django Views --------------------
@csrf_exempt
def indeed_chat(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            user_message = data.get('prompt', '')
            if user_message:
                response = get_indeed_response(user_message)
                save_conversation(user_message, response)
                return JsonResponse({'text': response})
        except Exception as e:
            logging.error(f"Indeed chat error: {e}")
    return JsonResponse({'text': 'Invalid request'}, status=400)

@csrf_exempt
def gmtt_chat(request):
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            user_message = data.get('prompt', '')
            if user_message:
                response = get_gmtt_response(user_message)
                return JsonResponse({'text': response})
        except Exception as e:
            logging.error(f"GMTT chat error: {e}")
    return JsonResponse({'text': 'Invalid request'}, status=400)

def indeed_view(request):
    return render(request, 'chatbot.html')

def gmtt_view(request):
    return render(request, 'give_me_tree.html')
